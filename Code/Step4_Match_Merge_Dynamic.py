...
# MAGIC
# MAGIC ## Performance fixes (this pass)
# MAGIC Slow even on tiny data almost always means the query **plan** is huge,
# MAGIC not the data. Three fixes applied:
# MAGIC 1. **Connected-components loop (§4.11) had no lineage break.** Each of
# MAGIC    up to 20 iterations built on the *previous* iteration's full plan
# MAGIC    without materializing it — so by iteration 20, every `.count()`
# MAGIC    re-derived all 19 prior iterations from scratch. Added
# MAGIC    `.localCheckpoint(eager=True)` every pass, which forces materialization
# MAGIC    and truncates lineage back to a single leaf node before the next
# MAGIC    iteration starts. This is the dominant fix — total work now scales
# MAGIC    with iteration count, not its square.
# MAGIC 2. **`df_silver`/`df_gold`/`df_universe`/`df_pairs` were recomputed from
# MAGIC    scratch** on every reuse — each is read by 7 separate match-type joins
# MAGIC    plus `resolve_groups()` plus multiple progress-print `.count()` calls,
# MAGIC    with nothing cached in between. `.cache()` added on each, with
# MAGIC    `.unpersist()` at the end of every domain's loop iteration so memory
# MAGIC    doesn't accumulate across domains.
# MAGIC 3. **Default 200 shuffle partitions** turn every join/groupBy into 200
# MAGIC    mostly-empty tasks for a small batch — real scheduling overhead, paid
# MAGIC    on every one of the loop's iterations. Session-scoped
# MAGIC    `spark.sql.shuffle.partitions = 8` (with AQE on) added in §4.0b;
# MAGIC    raise it back up if a domain's batch regularly exceeds ~1M rows.
# MAGIC
...
# MAGIC %md
# MAGIC ## 4.0b — Performance: Shuffle Partition Tuning
# MAGIC Databricks' default `spark.sql.shuffle.partitions` is 200 — fine for large
# MAGIC tables, but for a small-to-medium batch (typical incremental MDM run) this
# MAGIC means every join/groupBy spins up 200 mostly-empty tasks, each with real
# MAGIC scheduling overhead. That overhead is paid **every iteration** of the
# MAGIC connected-components loop below (§4.11), compounding badly. Lowering it
# MAGIC for this notebook's session (not globally) removes that overhead; adjust
# MAGIC upward if a single domain's batch regularly exceeds ~1M rows.

# COMMAND ----------

spark.conf.set("spark.sql.shuffle.partitions", "8")
print(">>> spark.sql.shuffle.partitions set to 8 for this session (AQE is always enabled on Serverless).")
...
def resolve_groups(domain_code: str, df_universe, df_pairs):
    ...
    # Performance fix: label propagation is iterative and Spark is lazy, so
    # without breaking lineage each pass, df_labels' query plan grows by a
    # full join-pair every iteration — by iteration 20 the plan re-derives
    # ALL 19 prior iterations from scratch on every action, making total work
    # scale with the SQUARE of the iteration count regardless of row count.
    # localCheckpoint() materializes df_labels to disk/memory and truncates
    # its lineage back to a single leaf node before the next iteration starts.
    MAX_ITER = 20
    for i in range(MAX_ITER):
        ...
        df_labels_new = (
            ...
            .drop("new_label")
            .localCheckpoint(eager=True)  # <-- truncates lineage every pass; the key fix
        )
        changed = (
...
for row in domains_to_process:
    ...
    # df_universe is the `b` side of every match-type join (7x) plus
    # resolve_groups' node/edge construction.
